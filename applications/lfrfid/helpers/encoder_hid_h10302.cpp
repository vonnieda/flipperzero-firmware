...
void EncoderHID_H10302::init(const uint8_t* data, const uint8_t data_size) {
    ProtocolHID10302 hid;
    hid.encode(data, data_size, reinterpret_cast<uint8_t*>(&card_data), sizeof(card_data) * 3);

    card_data_index = 0;
}

void EncoderHID_H10302::write_bit(bool bit, uint8_t position) {
    write_raw_bit(bit, position + 0);
    write_raw_bit(!bit, position + 1);
}

void EncoderHID_H10302::write_raw_bit(bool bit, uint8_t position) {
    if(bit) {
...
void EncoderHID_H10302::get_next(bool* polarity, uint16_t* period, uint16_t* pulse) {
...
EncoderHID_H10302::EncoderHID_H10302() {
